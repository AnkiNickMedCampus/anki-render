...
export const FIELD_BLOCKS = new Set(['field', 'cloze', 'image', 'hint', 'type']);
let counter = 0;
 ...
 */
export function defaultLayout(fieldNames) {
    const [first, second, ...rest] = fieldNames;
    if (!first) {
        return emptyLayout();
    }
    const back = [{ id: newBlockId(), type: 'frontside' }, { id: newBlockId(), type: 'divider' }];
    if (second) {
 ...
 * keine Karten.
 */
export function defaultClozeLayout(fieldNames) {
    ...
    return {
        ...
        back: [
            ...
            ...rest.map((name) => ({ id: newBlockId(), type: 'hint', field: name, label: name })),
        ],
    };
}
...
function wrapIf(cond, field, inner) {
    return cond ? `{{#${field}}}${inner}{{/${field}}}` : inner;
}
/** Einen Block in Vorlagen-HTML übersetzen. */
export function compileBlock(b) {
    switch (b.type) {
        case 'field': {
            const f = ref(b.field);
            const cls = classes('nb nb-field', b.size && b.size !== 'normal' && `nb-size-${b.size}`, b.align === 'left' && 'nb-align-left', b.tone && b.tone !== 'normal' && `nb-tone-${b.tone}`);
            return wrapIf(b.hideIfEmpty !== false, f, `<div class="${cls}">{{${f}}}</div>`);
        }
        case 'cloze': {
            const f = ref(b.field);
            const cls = classes('nb nb-cloze', b.size && b.size !== 'normal' && `nb-size-${b.size}`, b.align === 'left' && 'nb-align-left');
        ...
        case 'text': {
            const cls = classes('nb nb-text', b.align === 'left' && 'nb-align-left', b.tone && b.tone !== 'normal' && `nb-tone-${b.tone}`);
...
export function compileLayout(layout) {
    const join = (blocks) => blocks.map(compileBlock).join('\n');
    return {
...
export function usedFields(layout) {
    const out = new Set();
