 ...
 */
export function convertAnkiMath(html) {
    const inline = (x) => `<span class="katex-inline">${escapeHtml(x.trim())}</span>`;
    const block = (x) => `<span class="katex-block">${escapeHtml(x.trim())}</span>`;
    return html
 ...
 */
export function buildCardDocument(body, skin, opts = {}) {
